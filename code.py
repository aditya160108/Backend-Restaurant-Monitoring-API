...
def load_data():
    ...
    with open("store_status.csv", "r") as f:
        ...
        for row in reader:
            store_id, timestamp_utc, status = map(str.strip, row)
            c.execute("INSERT INTO store_status VALUES (?, ?, ?)", (store_id, timestamp_utc, status))
    ...
    with open("store_hours.csv", "r") as f:
        ...
        next(reader)  # Skip the header row
        for row in reader:
            store_id, dayOfWeek, start_time_local, end_time_local = map(str.strip, row)
            c.execute("INSERT INTO store_hours VALUES (?, ?, ?, ?)", (store_id, dayOfWeek, start_time_local, end_time_local))
    ...
    with open("store_timezones.csv", "r") as f:
        ...
        next(reader)  # Skip the header row
        for row in reader:
            store_id, timezone_str = map(str.strip, row)
            c.execute("INSERT INTO store_timezones VALUES (?, ?)", (store_id, timezone_str))
...
def generate_report():
    ...
    for store_id in store_ids:
        ...
        c.execute("SELECT dayOfWeek, start_time_local, end_time_local FROM store_hours WHERE store_id=?", (store_id,))
